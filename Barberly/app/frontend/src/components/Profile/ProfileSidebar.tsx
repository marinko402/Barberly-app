...
import { ClipboardClock, LogOut, Store, UserKey, UserPen } from "lucide-react";
...
const ProfileSidebar: FC = () => {
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex w-64 h-full bg-black/30 rounded-2xl backdrop-blur-xl border border-white/10 p-2 flex-col justify-between text-white shadow-2xl shrink-0">
              ...
              {visibleTabs.map((t) => {
                ...
                return (
                  <li
                    key={t.key}
                    onClick={() => goTo(t.key)}
                    className={`px-4 py-3 rounded-xl cursor-pointer transition-all duration-200 text-sm font-medium flex items-center gap-3.5 group ${
                      isActive
